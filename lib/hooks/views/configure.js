...
module.exports = function configure ( sails ) {
  ...
  if ( !sails.config.views.engine.fn ) {
    ...
    try {
      ...
      fn = getEngine(engineName,sails.config.views.engine.module, sails);
...
function getEngine(engineName, moduleName, sails) {
  ...
  if(!consolidate.requires[moduleName]) {
    try {
      ...
      consolidate.requires[moduleName] = require(moduleName);
      ...
      sails.log.info('Could not find module:', moduleName);
    }
  }
